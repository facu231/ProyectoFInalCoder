...
class TesteLoginApp_Codeer(unittest.TestCase):
    ...
    def test_login_logout(self):
        try:
            ...
            time.sleep(1)
            user.send_keys("foviedo")# Fossjblu
            password =  self.driver.find_element_by_xpath("//input[contains(@type,'password')]")
            time.sleep(1)
            password.send_keys("Mamamia1423") #kukumber Mamamia1423
            botton =  self.driver.find_element_by_xpath("//input[contains(@type,'submit')]")
            time.sleep(1)
            botton.click()
            time.sleep(3)
            modificarPerfil = self.driver.find_element_by_xpath("//span[contains(.,'Sugerencias')]")
            modificarPerfil.click()
            time.sleep(4)
            nombresCampos = ['user','project','detallles']
            datos = ['foviedo','Administrador de alumnos para escuela','Con la tecnologia que se le haga mas facil al desarrollador']
            ...
            modificar = self.driver.find_element_by_xpath("//button[contains(.,'Enviar Pedido')]")
            modificar.click()
            time.sleep(3)
